...
func ExecuteArgs(args []string, stdout, stderr io.Writer) int {
	...
	var ce *out.CLIError
	if errors.As(err, &ce) {
		if ce.Code == "watch_timeout" || ce.Code == "partial_failure" || ce.Code == "cache_differs" {
			// the verb already emitted its payload (watch's timeout line,
			// sync/push's ok:false envelope, or cache verify's difference
			// list) - a second error document would be a second write to the
			// same stream.
			return ce.ExitCode
		}
